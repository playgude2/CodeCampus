...
This is the `apps/api` workspace package in the [CodeCampus monorepo](../../README.md).
...
### Setup
Run `pnpm install` from the **repo root** (this is a pnpm workspace — one lockfile, one `node_modules` for every app). Everything else below can be run either from the repo root with `pnpm --filter @codecampus/api <script>`, or directly from `apps/api/` with the bare `pnpm <script>`.
...
pnpm --filter @codecampus/api migration:run    # apply schema
pnpm dev:api                                   # API on http://localhost:3000
pnpm --filter @codecampus/api start:worker:dev # judge worker (optional in dev — API also processes jobs)
```
...
Run any of these from `apps/api/` directly, or as `pnpm --filter @codecampus/api <script>` from the repo root.
